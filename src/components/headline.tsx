  ...
  return (
    <div className="rounded-lg bg-cream px-5 py-4">
      <div className="mb-2 flex items-start justify-between gap-3">
        {kicker ? (
          <p className="text-muted pt-1 font-ui text-xs font-semibold tracking-kicker uppercase">{kicker}</p>
        ) : (
          <span />
        )}
...
export function Headline({ kicker, text }: { kicker?: string; text: string }) {
  return (
    <div className="rounded-lg bg-cream px-5 py-4">
      {kicker ? (
        <p className="text-muted mb-2 font-ui text-xs font-semibold tracking-kicker uppercase">{kicker}</p>
      ) : null}
      <p className="font-display text-title font-medium tracking-[-0.02em] text-navy leading-[1.3]">{text}</p>
    </div>
  );
...
export function Stat({
  ...
  return (
    <div className="min-w-0">
      <div className="text-muted font-ui text-xs font-medium tracking-kicker uppercase">{label}</div>
      <div className={`tabular mt-1 font-display text-2xl font-medium tracking-tight ${color}`}>{value}</div>
      {hint ? <div className="text-muted mt-1 text-xs leading-snug">{hint}</div> : null}
    </div>
  );
}
